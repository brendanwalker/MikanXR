...
- The Graph Assets panel's New Material button opens the material editor on a fresh graph in the outer editor's domain. On its first save the outer graph gains a `MaterialAssetReference` to the written `.mat`, and the project catalog rescans so the material shows in every Assets panel. The compositor and shape editors' Graph Assets panels are the project catalog filtered to materials and textures, the material editor's to textures; a tile dropped on a canvas is found-or-added in that graph's own reference list ([scripting.md](./scripting.md)).
