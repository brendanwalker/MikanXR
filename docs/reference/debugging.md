...
The localization module reads `resources/localization` relative to the working directory, so run `MikanCmd.exe` from the repo root (as the commands in [commands.md](./commands.md) do) or the tables will not be found. The same holds for `Mikan.exe`: `PathUtils::getResourceDirectory` is the working directory plus `resources`, so an editor launched from its build folder reads whatever copy of the resources sits there, which is stale as soon as a localization key or a bundled asset changes. Descriptor labels rendering as raw `section.key` text is the symptom. Only one editor runs at a time: a second `Mikan.exe` fails in `CefInitialize` with `Opening in existing browser session` while the first holds CEF's user data directory, so an automation drive against a fresh build needs any open editor closed first.
...
- **Automation server.** A loopback TCP text command channel on port 21120 for driving and inspecting a running `Mikan.exe`: stage control, property read/write, function invokes, screenshots, Lua eval, and log tail. Off by default, so launch with `-automationServer` or turn it on in the project settings panel. See [automation.md](./automation.md).
...
- **Lua errors.** Every script failure is logged with its Lua traceback and opens the script error dialog in the editor, where an entry with a resolved file and line opens the code editor there. A load error (a file that fails to run, defines no `ScriptBehavior` class, or throws in `init`) disposes the whole script state; a trigger, HTTP trigger, message, sequence, or coroutine error leaves it running. Details in [scripting.md](./scripting.md).
