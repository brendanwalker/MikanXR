...
Depth testing for a shape draw is decided by the pass, not by the graph. `renderShapeGraph` takes a scene-depth-pass flag that reaches the nodes as `NodeEvaluator::setIsSceneDepthPass`, and `DrawShapeMeshNode` depth tests whenever it is set. The scene view sets it, because a shape there has to sort against the rest of the scene or the stencils and debug grid issued after it paint straight over. The compositor does not, leaving the node's own `depth_test` in charge, because a shape in a composite is often deliberately an overlay sitting on top of every layer. In the compositor that setting tests against the working buffer's depth, which starts at far each frame and is populated only by a `WriteDepthNode` earlier in the chain ([compositor.md](./compositor.md)).
