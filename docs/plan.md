...
- [ ] ARKit video source: re-verify the receive path at the restored 1s keyframe interval. Four times as many keyframes means four times as many large RTP bursts, so this is a real re-test of the `udpsrc` receive buffer rather than a formality. The 4s measurement was zero loss across 6621 frames.
