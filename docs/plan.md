...
- [ ] ARKit debug side channel: the transport, the `arkit` automation namespace, and the iOS client have landed and are verified against `tools/arkit_debug_stub.py`, but not yet against a real phone. Confirm the handshake, the log relay onto the shared timeline, and `arkit send stats` against a device, then use it for the frame-rate hitch investigation.
