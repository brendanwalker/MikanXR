...
- [ ] Node editor migration closeout (`node-zoom` branch): the code migration to imgui-node-editor is complete; the last remnant is dropping the `thirdparty/imnodes` submodule (`git rm` plus its `.gitmodules` entry).
