...
- [ ] ARKit marker tracking origin: the editor-side alignment landed (Align Camera solves an ARKit-world-to-stage offset from the origin marker and applies it in `notifyFrameBundleReceived`, see videosources.md), verified end to end on the software decode tier. Still to confirm on the hardware decode tier, which is the one that exercises `readbackDirectColorTexture`: the local GStreamer nvcodec pipeline has been refusing to build in-process, so every run fell back to `openh264dec` and the readback branch was never entered. Also unverified: moving the phone mid-sampling, which is the test that the per-sample composition beats naive averaging, and the `frameSeq` staleness guard clearing the offset after a phone restart.
...
- [ ] The align stage's test view stretches the video to the window while the axes overlay projects at the camera's own aspect ratio, so the two disagree by a few pixels on a 4:3 source in a 16:9 window. Cosmetic, but it makes the overlay look slightly misaligned when it is not.
