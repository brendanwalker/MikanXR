...
- [ ] Node editor migration (`node-zoom` branch): the editors now run on imgui-node-editor (pthom fork, `MikanXR/imgui-node-editor` submodule on `mikanxr-patches`) through the `MkCanvas` facade in MikanGUI, with wheel zoom, crisp text at zoom, and live link accept/reject feedback. Remaining: Phase 4 chrome polish (per-category header colors/icons, link preview coloring, output pin column alignment) and Phase 5 closeout (delete the `MkNodesScoped*` wrappers, ImNodes CMake wiring and submodule, the spike scaffolding: `NodeZoomSpikeWindow`, the `spike` automation namespace).
