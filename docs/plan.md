...
- [ ] ARKit marker tracking origin: the editor-side alignment landed (Align Camera solves an ARKit-world-to-stage offset from the origin marker and applies it in `notifyFrameBundleReceived`, see videosources.md). Verified on both decode tiers, including the hardware tier that actually exercises `readbackDirectColorTexture`, and three consecutive alignments with the phone moved between them agree on the offset to well under a millimetre, which is what confirms the per-sample composition. Still unverified: the `frameSeq` staleness guard clearing the offset after a phone restart.
