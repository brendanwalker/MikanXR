...
- [ ] ARKit video source: the ~110ms once-per-keyframe decoder stall is still unexplained, and is receive side. The send path is now instrumented and exonerated: a keyframe is 276 packets against 41 for a P frame, `StreamingSession` enqueues the whole burst in 0.4ms with a worst-case datagram queue wait of 1.7ms on an even 33ms cadence, and on the wire that burst transmits in 5 to 8ms at over 300Mbps, arriving on time. The link is nowhere near saturated, so send pacing would spread a burst that is not the problem.
	Ruled out by measurement, all reproducing in a standalone `gst-launch` pipeline with `fakesink` and no editor involved: appsink queue depth, the redundant NV12 conversion pass, caps renegotiation, decoder re-initialisation, `num-output-surfaces`, `max-display-delay`, keyframe size, the jitter buffer (unchanged at 50/200/500ms and with the element removed entirely), and the phone's send path.
	Two measurement traps to avoid repeating. The decoder's stall duration is a poor proxy for the wire gap because it quantises it, so wire timing must be measured with a raw socket rather than inferred from decode intervals. And one packet capture showed 35 gaps over 50ms while a later one under the same nominal conditions showed one, so wire gaps here are intermittent and a single capture cannot carry a conclusion. The decoder stall meanwhile is stable at 0.58 to 0.64/s with p99 109ms across every run, including runs where the wire was clean, which is what localises it to the receive pipeline.
	Useful next angle: the decoder emits in clumps (about 30 percent of output intervals are under 2ms) while its input is smooth, so something downstream of the depayloader is buffering and releasing in bursts.
