...
- [ ] Node editor zoom (`node-zoom` branch): the imgui-node-editor spike passed on ImGui 1.92.9 (wheel zoom, pan, node drag, link creation, in-node widgets, and crisp text at zoom via `SetFontRasterizerDensity` tied to canvas scale, all in a per-window ImGui context alongside the other editor windows). Decide whether to migrate the node editors off ImNodes to pthom's `imgui_bundle` fork of imgui-node-editor. Known costs: the fork expects imgui_bundle's patched ImGui, bridged for the spike by two shims in the untracked `thirdparty/imgui-node-editor` clone (context-hook aliases that reduce in-canvas popup handling to manual `ed::Suspend`/`Resume`, and two no-op diagnostic stubs); a real adoption picks between keeping those shims or a small ImGui submodule patch. Spike scaffolding to delete with the verdict: `NodeZoomSpikeWindow`, the `spike` automation namespace, `spikeSetFontRasterizerDensity`, the CMake wiring.
