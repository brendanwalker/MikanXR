...
- [ ] ARKit video source: every keyframe costs a delay hiccup on the receive side. At 1920x1440/30fps with a 1s keyframe interval, all 86 measured gaps over 100ms resumed at `frameSeq % 30 == 2` (one residue out of thirty) and were spaced a median of exactly 1.00s apart, so they are phase locked to the keyframe cadence. No frames are lost, they arrive late, and the effect survives both appsink queue depths and gating the redundant NV12 conversion pass, so it is neither of those. The remaining suspect is per-IDR caps renegotiation in `h264parse`/`nvh264dec` (each IDR carries SPS/PPS), which is also the mechanism `H264Encoder`'s own comment guessed at years before there was a way to measure it.
