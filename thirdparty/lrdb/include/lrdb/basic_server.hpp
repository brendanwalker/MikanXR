...
class basic_server {
 public:
  ...
  /// @brief Control whether the first Lua line event blocks waiting for a
  /// debugger client to connect.  Defaults to true (original LRDB behaviour).
  /// Set to false when embedding in a host app that must not block its main
  /// loop — debugger clients can still connect and set breakpoints at any
