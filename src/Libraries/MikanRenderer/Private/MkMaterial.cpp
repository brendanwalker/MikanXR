...
MkMaterial::MkMaterial(const std::string& name, IMkShaderPtr program)
	: m_impl(new MkMaterialImpl())
{
	m_impl->name= name;
...
const std::string& MkMaterial::getName() const { return m_impl->name; }

const NamedValueTable<float>& MkMaterial::getFloatSources() const { return m_impl->floatSources; }

const NamedValueTable<glm::vec2>& MkMaterial::getFloat2Sources() const { return m_impl->float2Sources; }

const NamedValueTable<glm::vec3>& MkMaterial::getFloat3Sources() const { return m_impl->float3Sources; }

const NamedValueTable<glm::vec4>& MkMaterial::getFloat4Sources() const { return m_impl->float4Sources; }

const NamedValueTable<glm::mat4>& MkMaterial::getMat4Sources() const { return m_impl->mat4Sources; }

const NamedValueTable<IMkTextureConstPtr>& MkMaterial::getTextureSources() const { return m_impl->textureSources; }

void MkMaterial::setProgram(IMkShaderPtr program) { m_impl->program= program; }

IMkShaderPtr MkMaterial::getProgram() const { return m_impl->program.lock(); }

bool MkMaterial::setFloatBySemantic(eUniformSemantic semantic, float value)
{
	std::string uniformName;
	IMkShaderPtr program= getProgram();
	if (program != nullptr && program->getFirstUniformNameOfSemantic(semantic, uniformName))
	{
...
bool MkMaterial::setFloatByUniformName(const std::string uniformName, float value)
{
	...
	IMkShaderPtr program= getProgram();
	if (program != nullptr && program->getUniformDataType(uniformName, datatype)
...
bool MkMaterial::getFloatByUniformName(const std::string uniformName, float& outValue) const
{
	...
	IMkShaderPtr program= getProgram();
	if (program != nullptr && program->getUniformDataType(uniformName, datatype)
		&& datatype == eUniformDataType::datatype_float && m_impl->floatSources.tryGetValue(uniformName, outValue))
	{
		return true;
	}
...
bool MkMaterial::getVec2ByUniformName(const std::string uniformName, glm::vec2& outValue) const
{
	...
	IMkShaderPtr program= getProgram();
	if (program != nullptr && program->getUniformDataType(uniformName, datatype)
		&& datatype == eUniformDataType::datatype_float2 && m_impl->float2Sources.tryGetValue(uniformName, outValue))
	{
		return true;
	}
...
bool MkMaterial::getVec3ByUniformName(const std::string uniformName, glm::vec3& outValue) const
{
	...
	IMkShaderPtr program= getProgram();
	if (program != nullptr && program->getUniformDataType(uniformName, datatype)
		&& datatype == eUniformDataType::datatype_float3 && m_impl->float3Sources.tryGetValue(uniformName, outValue))
	{
		return true;
	}
...
bool MkMaterial::getVec4ByUniformName(const std::string uniformName, glm::vec4& outValue) const
{
	...
	IMkShaderPtr program= getProgram();
	if (program != nullptr && program->getUniformDataType(uniformName, datatype)
		&& datatype == eUniformDataType::datatype_float4 && m_impl->float4Sources.tryGetValue(uniformName, outValue))
	{
		return true;
	}
...
bool MkMaterial::getMat4ByUniformName(const std::string uniformName, glm::mat4& outValue) const
{
	...
	IMkShaderPtr program= getProgram();
	if (program != nullptr && program->getUniformDataType(uniformName, datatype)
		&& datatype == eUniformDataType::datatype_mat4 && m_impl->mat4Sources.tryGetValue(uniformName, outValue))
	{
		return true;
	}
...
MkScopedMaterialBinding MkMaterial::bindMaterial(BindUniformCallback callback) const
{
	bool bMaterialFailure= false;
	UniformNameSet unboundUniformNames;
	IMkShaderPtr program= getProgram();

	if (program != nullptr && program->bindProgram())
	{
		for (auto it= program->getUniformBegin(); it != program->getUniformEnd(); ++it)
		{
			...
			case eUniformDataType::datatype_float:
			{
				...
				{
					bindResult= program->setFloatUniform(uniformName, value) ? eUniformBindResult::bound
			...
			case eUniformDataType::datatype_float2:
			{
				...
				{
					bindResult= program->setVector2Uniform(uniformName, value) ? eUniformBindResult::bound
																			   : eUniformBindResult::error;
				}
			}
			break;
			case eUniformDataType::datatype_float3:
			{
				...
				{
					bindResult= program->setVector3Uniform(uniformName, value) ? eUniformBindResult::bound
																			   : eUniformBindResult::error;
				}
			}
			break;
			case eUniformDataType::datatype_float4:
			{
				...
				{
					bindResult= program->setVector4Uniform(uniformName, value) ? eUniformBindResult::bound
																			   : eUniformBindResult::error;
				}
			}
			break;
			case eUniformDataType::datatype_mat4:
			{
				...
				{
					bindResult= program->setMatrix4x4Uniform(uniformName, value) ? eUniformBindResult::bound
																				 : eUniformBindResult::error;
				}
			}
			break;
			case eUniformDataType::datatype_texture:
			{
				...
				{
					bindResult= program->setTextureUniform(uniformName) && texture->bindTexture(textureUnit)
