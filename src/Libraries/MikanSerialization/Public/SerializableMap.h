...
namespace Serialization NAMESPACE()
{
	...
	{
	public:
		struct Entry { t_key key; t_value value; };

		Map() noexcept : m_entries(nullptr), m_size(0), m_capacity(0) {}

		Map(const Map& other) : m_entries(nullptr), m_size(0), m_capacity(0)
		{
		...
		Map(Map&& other) noexcept
			: m_entries(other.m_entries), m_size(other.m_size), m_capacity(other.m_capacity)
		{
		...
		const void* getOrAddRawValueMutable(const t_key& key)
		{
			...
			if (idx < m_size) return &m_entries[idx].value;
	...
	private:
		std::size_t findIndex(const t_key& key) const
		{
