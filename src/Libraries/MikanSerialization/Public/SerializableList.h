...
namespace Serialization NAMESPACE()
{
	...
	{
	public:
		List() noexcept : m_data(nullptr), m_size(0), m_capacity(0) {}

		List(std::initializer_list<T> init) : m_data(nullptr), m_size(0), m_capacity(0)
		{
			reserve(init.size());
			for (const T& item : init) push_back(item);
		}

		List(const List& other) : m_data(nullptr), m_size(0), m_capacity(0)
		{
			reserve(other.m_size);
			for (std::size_t i = 0; i < other.m_size; ++i) push_back(other.m_data[i]);
		}
		...
		void assign(const T* first, const T* last)
		{
			...
			for (std::size_t i = 0; i < count; ++i) m_data[i] = first[i];
			m_size = count;
		}
