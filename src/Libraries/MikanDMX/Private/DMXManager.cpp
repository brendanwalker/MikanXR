...
// -- IDMXManager factory -----
std::unique_ptr<IDMXManager> IDMXManager::create()
{
	return std::make_unique<DMXManager>();
}
...
{
	return m_sendThread.start(
		config.networkInterfaceIP,
		config.sourceName,
		config.priority,
		config.transmitRateHz);
}

void DMXManager::shutdown()
{
	m_sendThread.stop();
}

bool DMXManager::getIsRunning() const
{
	return m_sendThread.isRunning();
}
...
void DMXManager::setUniverseData(
	uint16_t universe,
