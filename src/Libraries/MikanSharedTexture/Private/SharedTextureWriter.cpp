...
private:
	...
	union
	{
		...
		class SpoutVulkanTextureWriter* spoutVulkanTextureWriter;
	} m_writerApi;
...
class SpoutVulkanTextureWriter
{
public:
	SpoutVulkanTextureWriter(SharedTextureWriteAccessor* parentAccessor)
		: m_parentAccessor(parentAccessor)
		, m_logger(parentAccessor->getLogger())
		, m_deviceTable()
	{
	...
	{
		const SharedTextureDescriptor* descriptor= m_parentAccessor->getRenderTargetDescriptor();
		const auto* vulkanInterface= (const SharedVulkanDeviceInterface*)m_parentAccessor->getApiDeviceInterface();
		...
		{
			if (!initSpoutFrame(m_spoutColorFrame, m_parentAccessor->getColorSenderName(), dxgiFormat)
		...
		if (descriptor->depth_buffer_type != SharedDepthBufferType::NODEPTH)
		{
			...
			if (!initSpoutFrame(m_spoutDepthFrame, m_parentAccessor->getDepthSenderName(), DXGI_FORMAT_R8G8B8A8_UNORM))
			{
				m_logger.log(SharedTextureLogLevel::error,
		...
		if (descriptor->shadow_buffer_type != SharedShadowBufferType::NOSHADOW)
		{
			if (!shadowBufferFormats(descriptor->shadow_buffer_type, dxgiFormat, vkFormat)
				|| !initSpoutFrame(m_spoutShadowFrame, m_parentAccessor->getShadowSenderName(), dxgiFormat)
	...
	bool writeColorFrameTexture(const SharedVulkanTexture* texture)
	{
	...
	bool writeDepthFrameTexture(const SharedVulkanTexture* texture, float zNear, float zFar)
	{
	...
	bool writeShadowFrameTexture(const SharedVulkanTexture* texture)
	{
	...
	void* getPackDepthTextureResourcePtr() const { return nullptr; }
											...
											void* apiDeviceInterface, void* apiCommandQueueInterface)
{
	...
	else if (descriptor->graphicsAPI == SharedClientGraphicsApi::Vulkan)
	{
		m_writerApi.spoutVulkanTextureWriter= new SpoutVulkanTextureWriter(this);
		m_graphicsAPI= SharedClientGraphicsApi::Vulkan;

		m_bIsInitialized= m_writerApi.spoutVulkanTextureWriter->init();
	}
...
void SharedTextureWriteAccessor::dispose()
{
	...
	else if (m_graphicsAPI == SharedClientGraphicsApi::Vulkan)
	{
		if (m_writerApi.spoutVulkanTextureWriter != nullptr)
		{
			m_writerApi.spoutVulkanTextureWriter->dispose();
			delete m_writerApi.spoutVulkanTextureWriter;
			m_writerApi.spoutVulkanTextureWriter= nullptr;
		}
	}
...
bool SharedTextureWriteAccessor::writeColorFrameTexture(void* apiTexturePtr)
{
	...
	{
		const SharedVulkanTexture* vulkanTexture= (const SharedVulkanTexture*)apiTexturePtr;

		bSuccess= m_writerApi.spoutVulkanTextureWriter->writeColorFrameTexture(vulkanTexture);
	}
	else
	{
		bSuccess= false;
...
bool SharedTextureWriteAccessor::writeDepthFrameTexture(void* apiTexturePtr, float zNear, float zFar)
{
	...
	else if (m_graphicsAPI == SharedClientGraphicsApi::Vulkan)
	{
		...
		bSuccess= m_writerApi.spoutVulkanTextureWriter->writeDepthFrameTexture(vulkanTexture, zNear, zFar);
	}
	else
	{
		bSuccess= false;
...
bool SharedTextureWriteAccessor::writeShadowFrameTexture(void* apiTexturePtr)
{
	...
	else if (m_graphicsAPI == SharedClientGraphicsApi::Vulkan)
	{
		...
		bSuccess= m_writerApi.spoutVulkanTextureWriter->writeShadowFrameTexture(vulkanTexture);
	}
	else
	{
		bSuccess= false;
...
void* SharedTextureWriteAccessor::getPackDepthTextureResourcePtr() const
{
	...
	{
		return m_writerApi.spoutVulkanTextureWriter->getPackDepthTextureResourcePtr();
	}
	else
	{
		return nullptr;
