...
void NodeEditorWindow::renderMainFrame()
{
	...
	for (auto& node : m_Nodes)
	{
		...
		if (node->type == EditorNodeType::EVENT)
		{
			...
			EditorEventNodePtr eventNode = (EditorEventNodePtr)node;
		...
		if (node->type == EditorNodeType::IMAGE)
		{
			...
			EditorImageNodePtr imgNode = std::static_pointer_cast<EditorImageNodePtr>(node);
			...
			else if (pin->type == EditorPinType::FLOW && m_Programs.size() > 0)
			{
				for (int i = 0; i < m_Programs.size(); i++)
				{
					if (ImGui::MenuItem(m_Programs[i]->GetName().c_str()))
					{
						...
						EditorProgramNodePtr newNode = (EditorProgramNodePtr)m_Nodes.back();
						if (pin->isOutput)
...
void NodeEditorWindow::DeleteProgram(int ix)
{
	...
	for (auto& node : m_Nodes)
	{
		if (node->type == EditorNodeType::PROGRAM)
		{
			EditorProgramNodePtr progNode = std::static_pointer_cast<EditorProgramNode>(node).get();
...
void NodeEditorWindow::DeleteTexture(int ix)
{
	...
	for (auto& node : m_Nodes)
	{
		if (node->type == EditorNodeType::TEXTURE)
		{
			EditorTextureNodePtr texNode = (EditorTextureNodePtr)node.get();
	...
	for (auto& pin : progNode->pinsIn)
	{
		...
		else if (pin->type == EditorPinType::TEXTURE)
		{
			if (pin->connectedLinks.size() > 0)
			{
				...
				{
					auto connectedProgNode = (EditorProgramNodePtr)connectedNode;
				...
				{
					auto texNode = (EditorTextureNodePtr)connectedNode;

					texture = texNode->target->GetTexture();
				}
				else if (connectedNode->type == EditorNodeType::IMAGE)
				{
					auto imgNode = (EditorImageNodePtr)connectedNode;

					if (imgNode->pinsIn[0]->connectedLinks.size() > 0)
					{
						auto texNode = (EditorTextureNodePtr)
		...
		else if (pin->type == EditorPinType::IMAGE)
		{
			if (pin->connectedLinks.size() > 0)
			{
				...
				if (connectedNode)
				{
					EditorImageNodePtr imgNode = (EditorImageNodePtr)connectedNode;

					if (imgNode->pinsIn[0]->connectedLinks.size() > 0)
					{
						EditorTextureNodePtr texNode = (EditorTextureNodePtr)
							GetConnectedPin(imgNode, imgNode->pinsIn[0]->connectedLinks[0])->pNode;
		...
		{
			EditorBlockPinPtr p = (EditorBlockPinPtr)pin;
			if (pin->connectedLinks.size() > 0)
			{
				EditorNodePtr connectedNode = GetConnectedPin(progNode, pin->connectedLinks[0])->pNode;
				...
				if (connectedNode)
				{
					EditorBlockNodePtr blockNode = (EditorBlockNodePtr)
					...
					if (p->blockPinType == EditorBlockPinType::UNIFROM_BLOCK)
					{
						...
						for (int i = 0; i < blockNode->pinsIn.size(); i++)
						{
							if (blockNode->pinsIn[i]->type == EditorPinType::FLOAT)
							{
								...
								{
									auto blockPin = (EditorFloatPinPtr)blockNode->pinsIn[i];
