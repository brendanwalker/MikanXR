 ...
 */
MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_Initialize(MikanLogLevel min_log_level, MikanLogCallback log_callback);

MIKAN_CLIENT_FUNCTION(int) Mikan_GetCoreSDKVersion();

MIKAN_CLIENT_FUNCTION(const char *) Mikan_GetClientUniqueID();

MIKAN_CLIENT_FUNCTION(bool) Mikan_GetIsInitialized();

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_SetGraphicsDeviceInterface(
	MikanClientGraphicsApi api, 
	void* graphicsDeviceInterface);

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_GetGraphicsDeviceInterface(
	MikanClientGraphicsApi api, 
	void** outGraphicsDeviceInterface);

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_AllocateRenderTargetTextures(
	const MikanRenderTargetDescriptor* descriptor, MikanRequestID* out_request_id);

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_FreeRenderTargetTextures(MikanRequestID* out_request_id);

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_WriteColorRenderTargetTexture(void* color_texture);

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_WriteDepthRenderTargetTexture(void* depth_texture, float z_near, float z_far);

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_PublishRenderTargetTextures(MikanClientFrameRendered* frame_info);

MIKAN_CLIENT_FUNCTION(void*) Mikan_GetPackDepthTextureResourcePtr();

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_SetClientInfo(const char* clientInfo);
 ...
 */
MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_Connect(const char* host, const char* port);

/** \brief Get the client connection status
    \return true if the client is connected to MikanXR Client API
 */
MIKAN_CLIENT_FUNCTION(bool) Mikan_GetIsConnected();
...
MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_FetchNextEvent(
...
MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_SendRequest(
	const char* utf8_request_name, 
	const char* utf8_payload,
	int request_version,
	MikanRequestID* out_request_id);

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_SetTextResponseCallback(
	MikanTextResponseCallback callback, 
	void* callback_userdata);

MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_SetBinaryResponseCallback(
...
MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_Disconnect();
 ...
 */
MIKAN_CLIENT_FUNCTION(MikanResult) Mikan_Shutdown();
