...
void LuaVec3f::bindFunctions(lua_State* L)
{
	luabridge::getGlobalNamespace(L)
		...
		.addProperty("x", +[](const LuaVec3f* vec)
						  { return vec->x; },
					 +[](LuaVec3f* vec, float v)
					 { vec->x= v; })
		.addProperty("y", +[](const LuaVec3f* vec)
						  { return vec->y; },
					 +[](LuaVec3f* vec, float v)
					 { vec->y= v; })
		.addProperty("z", +[](const LuaVec3f* vec)
						  { return vec->z; },
					 +[](LuaVec3f* vec, float v)
					 { vec->z= v; })
...
void LuaQuatf::bindFunctions(lua_State* L)
{
	luabridge::getGlobalNamespace(L)
		...
		.addProperty("w", +[](const LuaQuatf* q)
						  { return q->w; },
					 +[](LuaQuatf* q, float v)
					 { q->w= v; })
		.addProperty("x", +[](const LuaQuatf* q)
						  { return q->x; },
					 +[](LuaQuatf* q, float v)
					 { q->x= v; })
		.addProperty("y", +[](const LuaQuatf* q)
						  { return q->y; },
					 +[](LuaQuatf* q, float v)
					 { q->y= v; })
		.addProperty("z", +[](const LuaQuatf* q)
						  { return q->z; },
					 +[](LuaQuatf* q, float v)
					 { q->z= v; })
