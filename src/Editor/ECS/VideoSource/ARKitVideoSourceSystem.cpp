...
ARKitVideoSourceSystem::ARKitVideoSourceSystem() {}

ARKitVideoSourceSystem::~ARKitVideoSourceSystem() { dispose(); }
	...
	if (m_arkitVideoManagerState == eARKitVideoManagerState::initializing)
	{
		if (m_arkitVideoManagerFuture.wait_for(std::chrono::seconds(0)) == std::future_status::ready)
		{
			...
			if (result.manager)
			{
				...
				MIKAN_LOG_INFO("ARKitVideoSourceSystem::update") << "ARKit video device manager is ready";
			...
			else
			{
				...
				MIKAN_LOG_ERROR("ARKitVideoSourceSystem::update") << "Async ARKit video device manager init failed";
			}
		}
	}
...
void ARKitVideoSourceSystem::addManagerReadyCallback(std::function<void()> callback)
{
...
bool ARKitVideoSourceSystem::ensureARKitDeviceManager()
{
	...
	if (envVar == nullptr)
	{
		MIKAN_LOG_WARNING("ARKitVideoSourceSystem::ensureARKitDeviceManager")
			<< "GStreamer not installed. Skipping ARKit video manager init.";
		// Not a failure of this scaffold - just no manager available
		m_arkitVideoManagerState= eARKitVideoManagerState::failed;
		return false;
	}

	// Launch async init to avoid blocking the main thread
	MIKAN_LOG_INFO("ARKitVideoSourceSystem::ensureARKitDeviceManager")
...
ARKitVideoSourceSystem::ARKitVideoDeviceManagerInitResult ARKitVideoSourceSystem::initARKitVideoDeviceManagerOnThread(
	const std::string& moduleName)
{
	...
	if (result.module)
	{
		MIKAN_LOG_INFO("ARKitVideoSourceSystem::initARKitVideoDeviceManagerOnThread") << "Loaded module " << moduleName;

		// Attempt to create a device manager
		result.manager= result.module->createARKitVideoDeviceManager();
		if (result.manager)
		{
			MIKAN_LOG_INFO("ARKitVideoSourceSystem::initARKitVideoDeviceManagerOnThread")
			...
			{
				MIKAN_LOG_WARNING("ARKitVideoSourceSystem::initARKitVideoDeviceManagerOnThread")
	...
	{
		MIKAN_LOG_ERROR("ARKitVideoSourceSystem::initARKitVideoDeviceManagerOnThread")
...
void ARKitVideoSourceSystem::disposeARKitVideoDeviceManager()
{
	if (m_arkitVideoDeviceManager)
	{
