									  ...
									  Stats& outStats)
{
	...
	// right and +Y runs UP, so (top-left, bottom-left, top-right) is
	// counter-clockwise as seen from the camera - front faces wound around the
	// toward-camera vertex normals, the standard OBJ convention. This is
	// load-bearing: the Unreal client copies indices verbatim through a
	// handedness-flipping coordinate conversion, so a mesh wound the other way
	// (the first version of this was) shows every surface as a back face.
	...
	for (int gridY= 0; gridY + 1 < gridHeight; ++gridY)
	{
		for (int gridX= 0; gridX + 1 < gridWidth; ++gridX)
		{
			...
			tryEmitTriangle(topLeft, bottomLeft, topRight);
			tryEmitTriangle(topRight, bottomLeft, bottomRight);
		}
	}
								 ...
								 const std::string& textureFileName)
{
	...
	// Sibling .mtl projecting the captured frame back onto the proxy. Written
	// first so a failure aborts before the .obj references it.
	std::filesystem::path mtlPath;
	if (!textureFileName.empty())
	{
		mtlPath= std::filesystem::path(path).replace_extension(".mtl");

		std::ofstream mtlFile(mtlPath);
		if (!mtlFile)
		{
			MIKAN_LOG_ERROR("DepthMeshGenerator::saveObj") << "Could not open " << mtlPath.string() << " for writing";
			return false;
		}
		mtlFile << "newmtl " << objectName << "\n";
		mtlFile << "Ka 1.0 1.0 1.0\n";
		mtlFile << "Kd 1.0 1.0 1.0\n";
		mtlFile << "Ks 0.0 0.0 0.0\n";
		mtlFile << "d 1.0\n";
		mtlFile << "illum 1\n";
		mtlFile << "map_Kd " << textureFileName << "\n";
		if (!mtlFile.good())
			return false;
	}
