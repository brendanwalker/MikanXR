...
namespace
{
	...
	public:
		...
		void visitStruct(Serialization::ValueAccessor const& a) override
		{
			...
			if (t == rfk::getType<MikanVector2f>()) record(a, MikanVariantType::VECTOR2F);
			else if (t == rfk::getType<MikanVector3f>()) record(a, MikanVariantType::VECTOR3F);
			else if (t == rfk::getType<MikanVector4f>()) record(a, MikanVariantType::VECTOR4F);
			else if (t == rfk::getType<MikanQuatf>()) record(a, MikanVariantType::QUATERNIONF);
			else if (t == rfk::getType<MikanMatrix4f>()) record(a, MikanVariantType::MATRIX4F);
			else if (t == rfk::getType<MikanVector2d>()) record(a, MikanVariantType::VECTOR2D);
			else if (t == rfk::getType<MikanVector3d>()) record(a, MikanVariantType::VECTOR3D);
			else if (t == rfk::getType<MikanVector4d>()) record(a, MikanVariantType::VECTOR4D);
			else if (t == rfk::getType<MikanQuatd>()) record(a, MikanVariantType::QUATERNIOND);
			else setError(StringUtils::stringify(
	...
	bool testSchemaEntry(const SchemaTestEntry& entry)
	{
		...
		if (instance == nullptr)
		{
			MIKAN_LOG_ERROR("testSchemaEntry") << 
				"[" << entry.label << "]" << 
				" FAILED to allocate values struct instance";
			return false;
		}
		...
		if (schemaVisitor.hasError())
		{
			MIKAN_LOG_ERROR("testSchemaEntry") <<
				"[" << entry.label << "]" <<
				"values struct has an unsupported field: " << schemaVisitor.getError();
			success = false;
		}
		...
		for (const auto& descriptorEntry : descriptorTypesByName)
		{
			if (schemaVisitor.fieldTypes.find(descriptorEntry.first) == schemaVisitor.fieldTypes.end())
			{
				MIKAN_LOG_ERROR("testSchemaEntry") <<
					"[" << entry.label << "]" <<
					"descriptor '" << descriptorEntry.first << "' " <<
					"(" << mikanVariantTypeToString(descriptorEntry.second) << ") " <<
					"has no field in the values struct";
				success = false;
			}
		}
