...
bool GuiPanel_RGBSpotLightComponent::init()
{
	return initTypedPropertyInterface<RGBSpotLightComponent>();
}
...
void GuiPanel_RGBSpotLightComponent::onConstruct()
{
	...
	m_entityAccessor->setPropertyRenderer(
		RGBSpotLightComponent::k_greenPropertyId,
		[this](const PropertyDescriptorConstPtr& /*desc*/) -> bool
		{
			RGBSpotLightComponentPtr comp = getRGBSpotLightComponent();
			if (!comp) return false;
			...
			{
				addDeferredGuiEvent([this, green]() {
					if (auto c = getRGBSpotLightComponent())
	...
	m_entityAccessor->setPropertyRenderer(
		RGBSpotLightComponent::k_bluePropertyId,
		[this](const PropertyDescriptorConstPtr& /*desc*/) -> bool
		{
			RGBSpotLightComponentPtr comp = getRGBSpotLightComponent();
			if (!comp) return false;
