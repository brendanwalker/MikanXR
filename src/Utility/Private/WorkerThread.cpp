...
void WorkerThread::startThread()
{
    if (!m_threadStarted)
    {
        ...
        MIKAN_LOG_INFO("WorkerThread::start") << "Starting worker thread: " << m_threadName;
		onThreadStarted();
...
void WorkerThread::stopThread()
{
    if (m_threadStarted)
    {
        if (!m_exitSignaled)
        {
            MIKAN_LOG_INFO("WorkerThread::stop") << "Stopping worker thread: " << m_threadName;
            ...
            MIKAN_LOG_INFO("WorkerThread::stop") << "Worker thread stopped: " << m_threadName;
			onThreadHaltComplete();
        }
        else
        {
            MIKAN_LOG_INFO("WorkerThread::stop") << "Worker thread already stopped: " << m_threadName;
        }
...
{
    ThreadUtils::setCurrentThreadName(m_threadName.c_str());
