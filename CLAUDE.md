...
- **[materials.md](./docs/reference/materials.md)**: the material system (the `.mat` format and its semantics, domains and vertex presets, the material node graph and its node set, the compiler and the shader writer seam, the material editor and its preview, how the other editors reach a material and follow its edits). Read it before touching materials, shaders, or the material graph. Keep it current as nodes, presets, or writers are added.
