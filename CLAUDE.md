...
- **[TOKEN_STATS.md](./TOKEN_STATS.md)**: the AI token usage of this project's development and the energy and water estimate derived from it. Its tables and the README badges are generated by `tools/token_stats.py` from `docs/ai-usage.json` and verified in CI, so never hand-edit the marked blocks. The prose around them is written by hand.
...
- **[localization.md](./docs/reference/localization.md)**: the localization pipeline (the `localization/` gettext sources vs the generated `resources/localization/` tables, `tools/localization.py`, the fuzzy-flag provenance model, what adding a language costs, and the CI gate in front of the CDN). Read it before touching UI text tooling. Keep it current as the pipeline evolves.
